...
def precision_at_k(retrieved, relevant, k):
    ...
    relevant_count = sum(
        1 for chunk_id in retrieved_k if chunk_id in relevant
    )
...
def main():
    ...
    average_precision = (
        sum(precision_scores) / question_count
    )

    average_recall = (
        sum(recall_scores) / question_count
    )

    average_mrr = (
        sum(mrr_scores) / question_count
    )

    average_ndcg = (
        sum(ndcg_scores) / question_count
    )
